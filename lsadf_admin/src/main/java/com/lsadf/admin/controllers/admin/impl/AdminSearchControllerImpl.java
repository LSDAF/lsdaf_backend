...
public class AdminSearchControllerImpl extends BaseController implements AdminSearchController {
    ...
    public ResponseEntity<GenericResponse<List<GameSave>>> searchGameSaves(Jwt jwt,
                                                                           SearchRequest searchRequest,
                                                                           List<String> orderBy) {
        List<GameSaveSortingParameter> gameSaveOrderBy = Collections.singletonList(GameSaveSortingParameter.NONE);
        if (orderBy != null && !orderBy.isEmpty()) {
            gameSaveOrderBy = orderBy.stream()
                    .map(GameSaveSortingParameter::valueOf)
                    .toList();
        }
        validateUser(jwt);
        try (Stream<GameSave> gameSaveStream = searchService.searchGameSaves(searchRequest, gameSaveOrderBy)) {
