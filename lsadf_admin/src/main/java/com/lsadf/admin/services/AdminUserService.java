...
public class AdminUserService implements ListService<User> {
    ...
    @NonNull
    @Nonnull
    public List<@Nonnull User> list(@NonNull Pageable pageable,
        ...
        List<UserSortingParameter> sortingParameters = pageable.getSort().stream()
                .map(UserSortingParameter::fromOrder)
                .toList();
