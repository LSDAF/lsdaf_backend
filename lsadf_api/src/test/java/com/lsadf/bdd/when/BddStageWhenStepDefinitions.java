...
import static com.lsadf.bdd.ParameterizedTypeReferenceUtils.buildParameterizedStageResponse;
...
public class BddStageWhenStepDefinitions extends BddLoader {
    ...
    public void when_we_want_to_set_the_stages_for_the_game_save_with_id_to_with_cache(String gameSaveId, boolean toCache, DataTable dataTable) {
    ...
    @When("^the user requests the endpoint to set the stages with the following StageRequest for the game save with id (.*)$")
    public void when_the_user_requests_the_endpoint_to_set_the_stages_with_the_following_StageRequest_for_the_game_save_with_id(String gameSaveId, DataTable dataTable) {
        var data = dataTable.asMaps(String.class, String.class);
        assertThat(data).hasSize(1);

        StageRequest request = BddUtils.mapToStageRequest(data.get(0));

        String fullPath = ControllerConstants.STAGE + ControllerConstants.Stage.GAME_SAVE_ID.replace("{game_save_id}", gameSaveId);
    ...
    public void when_the_user_requests_the_endpoint_to_get_the_stages_of_the_game_save_with_id(String gameSaveId) {
        String fullPath = ControllerConstants.STAGE + ControllerConstants.Stage.GAME_SAVE_ID.replace("{game_save_id}", gameSaveId);
        String url = BddUtils.buildUrl(this.serverPort, fullPath);
        try {
            JwtAuthentication jwtAuthentication = jwtAuthenticationStack.peek();
            ...
            ResponseEntity<GenericResponse<Stage>> result = testRestTemplate.exchange(url, HttpMethod.GET, request, buildParameterizedStageResponse());
            var body = result.getBody();
            responseStack.push(body);
            log.info("Response: {}", result);
        } catch (Exception e) {
